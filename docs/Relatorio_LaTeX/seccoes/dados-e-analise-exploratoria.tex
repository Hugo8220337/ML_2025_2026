...
\begin{itemize}
	...
	\item \textbf{WELFake (Para o Modelo Final):}
	\begin{itemize}
		\item \textit{Origem:} IEEE Transactions on Computational Social Systems (Verma et al., 2021).
		\item \textit{Conteúdo:} Uma agregação de quatro \textit{datasets} (Kaggle, McIntire, Reuters e BuzzFeed Political), totalizando mais de 72.000 notícias com uma distribuição equilibrada entre reais e falsas.
		\item \textit{Justificação:} Foi selecionado por ser um dos maiores e mais diversos conjuntos de dados públicos. Ao combinar quatro fontes diferentes, evita o enviesamento típico de \textit{datasets} singulares e previne o \textit{overfitting}, garantindo que o Modelo Final é testado num cenário mais realista e genérico.
	\end{itemize}
\end{itemize}
...
\subsubsection{Análise de Anomalias (ISOT)}
O processamento deste conjunto de dados foi adaptado para suportar duas fases distintas de experimentação. Inicialmente, procedeu-se à fusão dos ficheiros originais (\textit{True.csv} e \textit{Fake.csv}) e à criação de uma coluna alvo (\textit{label}) para identificar a veracidade.

Posteriormente, a amostragem dos dados variou consoante a estratégia de modelação:

\begin{itemize}
	\item \textbf{Contaminação Controlada (Deteção de Anomalias):} Para os modelos não supervisionados, construiu-se um conjunto de treino composto pela totalidade das notícias verdadeiras, contaminado deliberadamente com apenas 5\% de notícias falsas. O objetivo foi simular se os algoritmos conseguiam isolar a minoria falsa como \textit{outliers};
	\item \textbf{Classificação Supervisionada:} Para os classificadores supervisionados (implementados após a falha da deteção de anomalias), utilizou-se a união completa de ambos os ficheiros, permitindo aos modelos aprender explicitamente as características distintivas de ambas as classes com a totalidade da informação disponível.
\end{itemize}
